...
(deftest history-test
  (let [db-spec {:path (str "/tmp/refdb-" (+ 5000 (int (rand 100000))))}]
    (dosync (ref-set coll1 nil))
    (is (nil? (db/init! db-spec coll1)))
    (let [{id :id :as saved} (first (db/save! db-spec coll1 {:m 1}))]
      ...
      (dosync (ref-set coll1 nil))
      (db/init! db-spec coll1)
      (is (= [{:id id :m 1 :a 2} {:id id :m 1}]
             (map #(dissoc % :inst)
                  (db/history coll1 (first (db/find coll1 nil))))))
      (db/with-transaction db-spec ^:test123 testtrans
        (db/save! db-spec coll1 {:ttt 33 :ff 88}))
      (dosync (ref-set coll1 nil))
      (db/init! db-spec coll1)
      (let [record (first (db/find coll1 :ttt 33))
            trns (db/transaction db-spec record)]
        (is record)
        (is trns)
